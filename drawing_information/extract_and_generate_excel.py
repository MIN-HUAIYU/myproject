...
def extract_detailed_info(ocr_text, image_name):
    ...
    for pattern in title_patterns:
        ...
        if match:
            ...
            if title and not title.startswith('26'):  # 排除位号
                data["设备名称"] = title
                break

    # 提取设备位号 - 格式通常是 XXXXX-EXXXXX
    location_patterns = [
        r'(\d{5}-E\d{3}[A-Z]{2})',
        r'(\d{5}-[A-Z]\d{3}[A-Z]{2})'
    ]
    ...
    for pattern in weight_patterns:
        ...
        if match:
            ...
            if int(weight) > 100:  # 设备重量通常较大
                data["重量"] = weight + " kg"
                break

    # 提取热侧/冷侧介质
    # 通常在介质行中，格式为 "介质  名称  [热侧]  [冷侧]"
...
def create_vertical_excel(equipment_list):
    ...
    ws[f'A{total_row + 3}'] = "数据格式：第一列为字段名，后续列为各设备数据"

    # 保存文件
    output_file = "设备数据表_一上一下.xlsx"
    wb.save(output_file)
    return output_file
...
def main():
    ...
    if equipment_list:
        ...
        print(f"\n文件格式说明：")
        print("  [OK] 第一列：字段名称（产品编号、用户名称、设备名称等）")
        print("  [OK] 后续列：每一列代表一个设备的数据")
        print("  [OK] 字段与数值一一对应，上下对齐")
        print(f"  [OK] 共识别 {len(equipment_list)} 个设备")
    else:
